...
def model_fn(features, labels, mode, params):
    ...
    def add_fully_connected_layer(input_tensor, out_channels):
        layer = tf.layers.dense(input_tensor, out_channels)
        return layer
    ...
    #print('PREV TENSOR SHAPE:', tf.shape(prev_tensor).eval(), prev_tensor.shape)
    #exit()
    if mode == tf.estimator.ModeKeys.TRAIN:
        # 16 images with 16 channels each and size 28 x 28
        prev_tensor_flat = tf.reshape(prev_tensor, [16, 16*28*28])
    if mode == tf.estimator.ModeKeys.PREDICT:
        prev_tensor_flat = tf.reshape(prev_tensor, [1, 16*28*28])

    #print('PREV TENSOR FLAT SHAPE:', tf.shape(prev_tensor_flat).eval())

    # Add fully connected layer (instead of another convolutional) to make actual predictions
    fully_conn_class = add_fully_connected_layer(prev_tensor_flat, num_classes)
    print('FULLY CONN OUTPUT:', fully_conn_class)
...
##############################################################################################
    ...
    labels_onehot = tf.one_hot(flat_labels, num_classes)
...
#     flat_preds = flat_preds + epsilon
    ...
    #         labels = labels_onehot, logits = tf.argmax(fully_conn_class))
    #preds = tf.argmax(fully_conn_class)
    #labels = tf.cast(labels, dtype = tf.float32)
    #flat_preds = tf.reshape(preds, [-1, num_classes])

    #print('PREDS ARE:', preds)
    cross_entropy = tf.nn.softmax_cross_entropy_with_logits(
                            _sentinel=None,
                            labels=labels_onehot,
                            logits=fully_conn_class,
                            dim=-1,
                            name=None)
