...
# Artificial train images
artificial = {}
for digit in range(10):
    artificial[digit] = np.load("mnist_random_artificial\\new_images_" 
              + str(digit) + ".npy")

# Shape is now (28, 28, 1000)
train_tensor = np.dstack([np.dstack((train_dict[digit], artificial[digit])) for digit in train_dict.keys()])
train_tensor = np.moveaxis(train_tensor,  -1, 0)
# Shape is now (1000, 28, 28, 1)
...
learning_rate = 1e-5
bn_decay = 0.9
model_dir_name = "MNIST_model_random_aug"
